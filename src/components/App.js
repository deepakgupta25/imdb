...
class App extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      this.setState({ movieList: respose, isLoading: false, error: "" });
    } catch (e) {
  ...
  initHomePageData = async () => {
    this.setState({ isLoading: true, searchKey: "" });
    try {
      const result = await post(null);
      const respose = await result.json();
      this.setState({ movieList: respose, isLoading: false, error: "" });
  ...
  handleSearchClick =  async (searchKey) => {
    this.setState({ isLoading: true });
    try {
      const postData = {
        search: this.getFormattedStr(searchKey),
        searchType: this.searchType || "movie"
      }
      const result = await post(postData);
      const respose = await result.json();
      this.setState({ movieList: respose, isLoading: false, error: "" });
    } catch (e) {
      this.setState({ isLoading: false, error: "Something Went Wrong !!!" })
  ...
  render() {
    const { error, isLoading, movieList } = this.state;

    return (
      <div className="App">
        <Header 
          onSearchClick={searchKey => this.handleSearchClick(searchKey)}
          onSearchTypeClick={searchType => this.handlesearchType(searchType)} 
          renderHomePageData={this.initHomePageData}
        />
        {isLoading ? <Loader /> : error ? <ErrorComponent errorMsg={error} /> : movieList.length > 0 ? (
          <div className="movie-listing-wrapper">
            <div className="sort-holder">
              <select className="sorting-dd" onChange={this.handleSorting}>
                <option value="" disabled selected>Sort By</option>
                <option value="popularity">Popularity</option>
                <option value="director">Director Name (A-Z)</option>
                <option value="name">Movie Name (A-Z)</option>
              </select>
            </div>
            <MovieListingPage movies={movieList} />
          </div>
